...
- **Mission/workflow graph preflight** (VS-007) — machine-facing gate (`preflightGraph()`) that consumes the same engine before execution: advisory in warn mode, blocks on blocking findings in enforce mode (typed `Mission`/`WorkflowPreflightError`). Exposed for execution paths to call at their boundary; not wired into writes.
- **Enterprise Governance Orchestrator** (VS-008 / ADR-0010) — single deterministic entry point (`evaluateGovernance()`) for missions/workflows/imports/API/agents/automation. Composes object + relationship + graph + preflight + policy stages into one `GovernanceDecision` (PASS / PASS_WITH_WARNINGS / BLOCKED); warn-default, opt-in enforce (`ONTOLOGY_GOVERNANCE_ENFORCEMENT`). Policy evaluation is an extension point (registry + interfaces only). On-demand; composes services without replacing them.
- **Mission Execution Runtime** (MS-010 / ADR-0012) — canonical deterministic engine (`executeMission()`) that runs enterprise missions after governance approval: execution planner + dependency resolver, task scheduler (sequential/parallel), agent dispatcher (executor registry), human approval gate (pause/resume), deterministic retries, typed runtime events, and a `MissionExecutionReport`. Governance decides; the runtime owns execution. Generic infrastructure only (no business logic); additive; no write-path changes.
- **Governed action executor + durable mission executions** (MS-011 / ADR-0013) — a generic `mission.action` executor bridges mission tasks to the Mission Control action engine (`executeAction`), inheriting its governed pipeline (idempotency/permission/ACL/approval/audit); every run is persisted as a queryable `missionExecutions` record. No business agents; additive; no write-path changes.
