// Phase 8 — onboarding demo runs through real services to completion.
...
test("onboarding demo completes with real artifacts", async () => {
  await resetDatabase();
  resetClock();
  const ctx = systemActor("tnt_test");
  const run = await runDemo(ctx, "onboarding", "day1-readiness");
