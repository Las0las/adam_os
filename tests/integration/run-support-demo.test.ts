// Phase 8 — support demo runs through real services to completion.
...
test("support demo completes with real artifacts", async () => {
  await resetDatabase();
  resetClock();
  const ctx = systemActor("tnt_test");
  const run = await runDemo(ctx, "support", "grounded-ticket-response");
