// Phase 8 — executive demo runs through real services to completion.
...
test("executive demo completes with real artifacts", async () => {
  await resetDatabase();
  resetClock();
  const ctx = systemActor("tnt_test");
  const run = await runDemo(ctx, "executive", "account-risk-to-decision-memo");
