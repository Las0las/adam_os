  ...
  return (
    <>
      ...
      <div className="grid grid-4">
        <Metric label="Sources" value={db.sources.list(ctx.tenantId).length} />
        <Metric label="Raw assets" value={db.rawAssets.list(ctx.tenantId).length} />
        <Metric label="Ontology objects" value={objects.length} />
        <Metric label="Evidence chunks" value={db.evidenceChunks.list(ctx.tenantId).length} />
      </div>
